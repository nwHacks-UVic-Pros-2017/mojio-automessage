...
class MojioUser {
    ...
    setup_ignition_event(vehicle_id, base_url, callback) {
        ...
        this._mojio_client.post("https://push.moj.io/v2/vehicles/" + vehicle_id + "?force=true", data).then(function(res,err) {
            if (err == null) {
                callback(key);
            } else {
                console.log(err);
    ...
    delete_observer(key, callback) {
        this._mojio_client.delete("https://push.moj.io/v2/vehicles/" + key).then(function(res, err) {
            console.log(res);
            if (err == null) {
                callback(true);
            } else {
